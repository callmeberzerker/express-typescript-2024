...
```jsx
// NOTE: needs to come from env or some other config and should not be present in the source code
...
// NOTE: no need for CJS in node, use ESM
...
app.post("/login", express.json(), async (req, res) => {
  ...
  // NOTE: needs to be validated against a schema
  // NOTE: the email needs to be normalized to lowercase
  let user = await db("users").where("email", email).first();

  if (!user) {
    // couldn't find user
    // NOTE: this reveals too much information about the user a 401 is the general practice for this scenario
    return res.status(404).end();
  }

  // NOTE: the password needs to be hashed and compared and not stored in plaintext as the example has done
...
app.get("/me", async (req, res) => {
  ...
  jwt.verify(authToken, AUTH_SECRET, async function (err, decoded) {
    if (err) {
      ...
      // NOTE: 401 is the general practice for this scenario
      return res.status(404).end();
    }
    ...
    // NOTE: this is very dangerous of spitting out the whole user object, better to just send back the data that is needed - cherrypicked and validated against a schema.
    res.send(user);
  });
});
